...
# Bot States
ADDING_MEASUREMENT, TYPING_REPLY, ADDING_MEASUREMENT_TYPE = range(3)

# Static vars
AT_INPUT_PATTERN = re.compile("^[^@]+@[^@]+$")
...
class MeasureBot(Bot):
        ...
        self.conv_handler = ConversationHandler(
            ...
            states={
                ADDING_MEASUREMENT: [RegexHandler('^([^:@]+:[^:@]+|[^@]+@[^@]+)$',
                                                       ...
                                                       pass_user_data=True),
                                          CommandHandler('undo_latest', self.undo_latest),
            ...
            fallbacks=[RegexHandler('^Check', self.check, pass_user_data=True)]
        )
    ...
    def add_measurement(self, bot, update, user_data):
        ...
        if COLUMN_INPUT_PATTERN.match(user_text):
            ...
            measurement = {'type': user_text_split[0].strip(), 'value': user_text_split[1].strip()}
        elif AT_INPUT_PATTERN.match(user_text):
            user_text_split = user_text.split("@")
            measurement = {'type': user_text_split[0].strip(), 'value': user_text_split[1].strip()}
        else:
        ...
        if measurement['type'] in user_data['known_types']:
            ...
            update.message.reply_text(
                'Measurement added: %s: %s. To undo it use command /undo_latest. To store another one, please type a new measurement in a format [measure]: [value].' % (
    ...
    def lazy_init_user_data(self, user_data):
        user_data['known_types'] = user_data[
            'known_types'] if 'known_types' in user_data else set()  # lazy-init known_types
    ...
    def add_measurement_type(self, bot, update, user_data):
        if not user_data['current_input']:
        ...
        if '@' in update.message.text or ':' in update.message.text:
            update.message.reply_text(
                'Measurement type value [%s] should not contain separator symbols (@:). Measurement type [%s] for value [%s] is new. To confirm please type it again. To correct it, write the correct value. To cancel, use command /cancel.' % (
                user_text, user_data['current_input']['type'], user_data['current_input']['value']))
            return ADDING_MEASUREMENT_TYPE
            ...
            update.message.reply_text(
                ...
                user_text, user_data['current_input']['type'], user_data['current_input']['value'], user_text))
    ...
    def unrecognised_format(self, bot, update):
        update.message.reply_text(
            '[%s] is in unrecognised format. Please type a new measurement in a format\n[measure]:[value]. For example,\nCafe Spending: 4$\nMarc weight: 78\nRun: 8.5km' % update.message.text)
    ...
    def check(self, bot, update, user_data):
        ...
        update.message.reply_text("I learned these facts about you:"
                                  "%s"
                                  "Until next time!" % self.facts_to_str(user_data))
    ...
    def undo_latest(self, bot, update):
        update.message.reply_text(
            "Latest measurement got removed: [measure]: [value]. To store correct one, please type a new measurement in a format [measure]: [value].",)
